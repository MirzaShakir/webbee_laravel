...
class MenuController extends BaseController
{
    ...
    public function getMenuItems() {
		$arrAllMenuItems = DB::select('select * from menu_items');
	    $arrAllMenuItems = json_decode(json_encode( $arrAllMenuItems ), true);

	    $arrReturnItems = [];
	    if( !empty( $arrAllMenuItems) ) {
		    $arrMenuItems = [];
		    foreach( $arrAllMenuItems as $arrMenuItem ) {
			    $arrMenuItems[ $arrMenuItem['id'] ] = $arrMenuItem;
		    }

		    $arrReturnItems = $this->menuItem->buildRelationship($arrAllMenuItems);
	    }

	    return response()->json($arrReturnItems);
    }
}
